...
describe("Message Converters", () => {
  ...
  describe("fromDisplayMessage", () => {
    ...
    it("should preserve previousMessageId", () => {
      const display: DisplayMessage = {
        id: "msg-1",
        type: "user",
        status: "complete",
        content: "Hello",
        previousMessageId: "parent-1",
      };

      const result = fromDisplayMessage(display);
      expect(result.previousMessageId).toBe("parent-1");
    });
  });
  ...
  describe("Round-trip conversion", () => {
    it("should maintain data integrity through InternalMessage → DisplayMessage → InternalMessage", () => {
      ...
      expect(restored.previousMessageId).toBe(original.previousMessageId);
