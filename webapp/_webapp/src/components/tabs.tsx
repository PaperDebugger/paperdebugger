...
export const Tabs = forwardRef<TabRef, TabProps>(
  ({ items }, ref) => {
    ...
    return (
      <>
        <div className={
          cn("pd-app-tab-items noselect transition-all duration-300",
            sidebarCollapsed ? "w-16" : "w-[140px]")
        }>
          {!hideAvatar && <Avatar name={user?.name || "User"} src={user?.picture} className="pd-avatar" />}

          <NextTabs aria-label="Options" isVertical variant="light" classNames={{
            tabList: "bg-gray-100",
            tab: "justify-start",
          }}
          ...
          <div className={
            cn(
              ...
              sidebarCollapsed ? "text-[8px]" : "text-md px-3"
            )
          }>
