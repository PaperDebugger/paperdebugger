...
type XtraMcpToolResult = {
  ...
  content?: string | object;  // string for verbatim, object for interpret
  metadata?: Record<string, any>;
  success?: boolean;
  error?: string;
};

type XtraMcpGenericCardProps = {
  functionName: string;
  message?: string;  // Raw tool result message (XtraMCPToolResult JSON from backend)
  preparing: boolean;
  animated: boolean;
};

export const XtraMcpGenericCard = ({ functionName, message, preparing, animated }: XtraMcpGenericCardProps) => {
  ...
  if (message) {
    try {
      ...
      // Validate it has display_mode to confirm it's ToolResult format
      // Backend go has schema enforcement, so this should be reliable
