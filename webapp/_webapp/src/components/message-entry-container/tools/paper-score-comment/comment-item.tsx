...
export const CommentItem = ({ comment, isSelected, onToggle }: CommentItemProps) => {
    ...
    const truncatedQuoteText = comment.quoteText && comment.quoteText.length > 100
    ...
    return (
        <div className={cn(
            ...
            isSelected ? "!bg-primary-50/50" : "!bg-white"
            ...
            <p className="!text-xs !text-gray-400">Position: {comment.docPath}:{comment.quotePosition}</p>
        </div>
    );
}; 