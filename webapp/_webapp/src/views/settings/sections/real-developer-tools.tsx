...
export const RealDeveloperTools = () => {
    const { showTool: enabled, setShowTool, slowStreamingMode, setSlowStreamingMode, alwaysSyncProject, setAlwaysSyncProject } = useDevtoolStore();

    return (
        <SettingsSectionContainer>
            <SettingsSectionTitle><b className="text-red-600">Real</b> Developer Tools *</SettingsSectionTitle>
            <SettingItemSelect
            ...
            <SettingItem
                ...
                onSelectChange={(selected) =>
                    setShowTool(selected)
                }
            />
            <SettingItem
                className={cn(
                    "transition-all duration-400",
                    slowStreamingMode ? "bg-purple-100" : "",
                )}
                ...
                onSelectChange={(selected) =>
                    setSlowStreamingMode(selected)
                }
            />
            <SettingItem
                className={cn(
                    "transition-all duration-400",
                    alwaysSyncProject ? "bg-purple-100" : "",
                )}
