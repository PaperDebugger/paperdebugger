...
import { MessageChunk, MessageTypeUserSchema, StreamError, StreamInitialization, StreamPartBegin, StreamPartEnd } from "../pkg/gen/apiclient/chat/v1/chat_pb";
...
export function useSendMessageStream() {
  ...
  const sendMessageStream = useCallback(async (message: string, selectedText: string) => {
    ...
    if (import.meta.env.DEV && alwaysSyncProject) {
      ...
      await sync(user?.id || "", getProjectId(), {
        ...
        createConversationMessageStream(request, async (response) => {
          switch (response.responsePayload.case) {
            case "streamInitialization": // means the user message is received by the server, can change the status to FINALIZED
              handleStreamInitialization(response.responsePayload.value as StreamInitialization, refetchConversationList);
              break;
            ...
            case "streamFinalization":
              ...
              break;
            case "streamError":
              await handleStreamError(response.responsePayload.value as StreamError, user?.id || "", message, selectedText, sync, sendMessageStream);
              break;
  ...
  }, [resetStreamingMessage, resetIncompleteIndicator, updateStreamingMessage, currentConversation, refetchConversationList, sync, user?.id, alwaysSyncProject, conversationMode]);
